...
- Que tan acertada es la percepción de las personas sobre su genero favorito?
	Para este ultimo vamos a extraer la intensidad promedio de los generos más populares del siguiente [estudio]( https://github.com/isaackwy/loudness-wars-spotify)
...
A continuación, vamos a mapear un género para cada usuario segun el volumen promedio de las canciones que escuchan. Luego, vamos a comparar el género mapeado con el género percibido como favorito por el usuario.
Para darle a cada usuario un genero estimado, vamos a asignarle aquel que tenga el volumen promedio más cercano al volumen promedio de las canciones que escucha segun la variable `average_genders_info`
...
En este caso de estudio vamos a analizar la relación entre el la corrección de la percepción de género y la cantidad de canciones escuchadas por año.
Es posible que un usuario tenga más claro su género favorito si escucha menos canciones por año.
Para esto vamos a realizar un test de independencia entre estas dos variables.
...
```
$H_0$: La cantidad de canciones escuchadas por año es independiente de la corrección de la percepción de género
$H_a$: La cantidad de canciones escuchadas por año no es independiente de la corrección de la percepción de género
...
$H_0$: La ubicación es independiente del genero preferido
$H_a$: La ubicación no es independiente del genero preferido
...
district_genre_df
# remove the categories with less than 10 observations
district_genre_df <- district_genre_df %>% filter(distrito != "Otros", distrito != "No especifica")
